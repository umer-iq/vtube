...
const Comments = ({ comment, setComment, addComment, comments, isPostingComment }: IProps) => {
  ...
  return (
    <div className='border-t-2 border-gray-200 pt-4 px-10 mt-4 bg-[#F8F8F8] border-b-2 lg:pb-0 pb-[100px]'>
      <div className='overflow-scroll lg:h-[300px]'>
        {comments?.length > 0 ? (
              ...
              {allUsers?.map(
                (user: IUser) =>
                  user._id === (item.postedBy._ref || item.postedBy._id) && (
                    <div className=' p-3 items-center' key={idx}>
     ...
     {userProfile && <div className='absolute bottom-0 left-0  pb-6 px-2 md:px-10 '>
        <form onSubmit={addComment} className='flex gap-100'>
          <input
            value={comment}
          ...
          />
          <button className='text-md text-black-400 ' onClick={addComment}>
